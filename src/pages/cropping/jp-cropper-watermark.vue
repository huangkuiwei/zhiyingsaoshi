...
<view v-show="url" :mode="modeValue" :change:mode="mwx.changeMode" :rotate="rotate" :change:rotate="mwx.changeRotate">
	...
	<view class="panel">
		<view class="body" @touchstart="mwx.touchstart" @touchmove="mwx.touchmove" @touchend="mwx.touchend" @touchcancel="mwx.touchcancel">
			...
			<view class="frame" :class="{transit: transit}" :change:rect="mwx.changeFrame" :rect="frame" v-show="step === 1 && showCropBox">
				<view class="rect">
    ...
    <view class="d-flex" style="padding: 0 2rem; height: 50px;">
      <view class="cropping-jp" style="flex-grow: 1">
        <template v-if="step === 1">
          <view @tap="onok" style="text-align: center;">
            <image
                src="https://hnenjoy.oss-cn-shanghai.aliyuncs.com/scantool/static/assets/my/xz1.png"
                style="height: 16px; width: 16px"
                mode="heightFix"
            />
            <view>裁剪</view>
          </view>
          <view @click="rotateAngle" style="margin: 0 1.5rem; text-align: center">
            <image
                src="https://hnenjoy.oss-cn-shanghai.aliyuncs.com/scantool/static/assets/my/xz2.png"
                style="height: 16px; width: 16px"
                mode="heightFix"
            />
            <view>左旋</view>
          </view>
          <view @click="rotateRightAngle" style="text-align: center">
            <image
                src="https://hnenjoy.oss-cn-shanghai.aliyuncs.com/scantool/static/assets/my/xz3.png"
                style="height: 16px; width: 16px"
                mode="heightFix"
            />
            <view>右旋</view>
        ...
        <template v-else>
          <view @tap="reAdd" style="text-align: center">
            <image
          ...
          <view @click="addWatermark" style="margin: 0 1.5rem; text-align: center">
            <image
                src="https://hnenjoy.oss-cn-shanghai.aliyuncs.com/scantool/static/assets/shuiyin.png"
                style="height: 16px; width: 16px"
                mode="heightFix"
            />
            <view>加水印</view>
      ...
      <!--<wd-button @click="getImg">保存</wd-button>-->
      <button class="submit" @click="submit" v-if="!showCropBox">确认</button>

      <template v-else>
        <button class="close" @click="$emit('update:showCropBox', false)" v-if="showCropBox">
          <wd-icon v-if="showCropBox" name="close"></wd-icon>
        </button>

        <button class="check" @click="submit">
          <wd-icon v-if="showCropBox" name="check"></wd-icon>
          <text v-else>确认</text>
        </button>
      </template>
    </view>
...
export default {
  ...
  emits: ['submit1', 'submit2', 'reAdd', 'update:showCropBox'],
	data() {
		return {
    ...
    submit() {
      if (this.showCropBox) {
        this.$emit('update:showCropBox', false)
    ...
    addWatermark() {
      toRouter('/pages/watermark/index', 'url=' + this.url + '&type=watermark&hideReply=true')
    },
		...
		onok() {
      this.$emit('update:showCropBox', true)
		...
		getImg(){
			...
			let that = this;
			uni.uploadFile({
			url: "https://tools-api.xiaoohui.com/api/oss/upload/form/nofiles",
			formData: {
